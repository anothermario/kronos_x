...
class BasicMomentumStrategy:
    ...
    def generate_signal(self, candle: Candle) -> Signal:
        ...
        if delta > self.threshold:
            return Signal(candle.symbol, "buy", 0.6, "close_above_open", candle.timestamp)
        if delta < -self.threshold:
            return Signal(candle.symbol, "sell", 0.6, "close_below_open", candle.timestamp)
        return Signal(candle.symbol, "hold", 0.5, "no_edge", candle.timestamp)
...
class BasicRisk:
    ...
    def to_order(self, signal: Signal, equity: float) -> Order | None:
        if signal.side == "hold":
            return None
        qty = max((equity * self.risk_fraction) / 100.0, 0.001)
